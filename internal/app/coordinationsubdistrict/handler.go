...
func (h *handler) Export(c *gin.Context) {
	...
	if err != nil {
		response := util.APIResponse("Failed to retrieve export coordination subdistrict: "+err.Error(), http.StatusInternalServerError, "failed", nil)
		c.JSON(http.StatusInternalServerError, response)
		return
	}

	fileName := fmt.Sprintf("export_kordes_%s.xlsx", time.Now().Format("20060102150405"))
